...
def continuous_learning_update(name, new_embedding, confidence, db_session):
    """Continuously improve embeddings with new successful recognitions"""
    try:
        from app0 import Person
        ...
        updated_template = {
            ...
            'update_count': person.update_count + 1
        }
        
        person.embedding = pickle.dumps(updated_template)
        person.last_updated = datetime.datetime.now()
        person.update_count = person.update_count + 1
        
        db_session.commit()
        
        # Rebuild FAISS periodically
        if updated_template.get('update_count', 0) % 50 == 0:
            rebuild_faiss_index(db_session)
...
def rebuild_faiss_index(db_session):
    ...
    persons = db_session.query(Person).filter(Person.status != "blocked").all()
    ...
    for person in persons:
        try:
            ...
            print(f"[Index Rebuild] Error processing {person.name}: {e}")
            continue
...
def search_face_faiss(embedding, threshold=0.6):
    """Enhanced FAISS search with continuous learning"""
    ...
    try:
        ...
        if best_name and best_score >= 0.75:
            continuous_learning_update(best_name, embedding, best_score, db)
...
def generate_camera_stream():
    """Enhanced camera stream with continuous learning"""
    ...
    while True:
        ...
        try:
            ...
            for face_obj in face_objs:
                ...
                if matched_name:
                    db = SessionLocal()
                    try:
                        from app0 import Person, Attendance
                        ...
                        update_count = person.update_count if person else 0
        ...
        # System info overlay
        cv.putText(frame, f"FAISS Index: {faiss_index.ntotal} faces", (10, 30),
